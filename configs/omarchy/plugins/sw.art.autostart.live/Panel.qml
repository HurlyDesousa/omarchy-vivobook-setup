...
Panel {
    ...
    function monitorStatus(mon) {
        ...
        if (mon.connected)
            return name
        if (mon.laptop)
            return name + " · unplugged"
        return name + " · unplugged, falls back to laptop"
    }

    function cycleMonitorWorkspace(mon, delta) {
        if (!mon || !mon.name)
            return
        var next = ((Number(mon.workspace) || 0) + delta + 10) % 10
        runMutate(["monitor-workspace", String(mon.name), String(next)])
    }
    ...
    KeyboardPanel {
        ...
        PanelKeyCatcher {
            ...
            Column {
                ...
                Item {
                    ...
                    Row {
                        ...
                        Rectangle {
                            ...
                            PanelToolTip {
                                visible: settingsClick.containsMouse
                                text: "Settings"
                                fontFamily: root.bar.fontFamily
                            }
                        }
                ...
                Column {
                    visible: root.showSettings && !root.picking
                    width: parent.width
                    ...
                    Text {
                        ...
                        text: "Pick the workspace that should live on each screen. If that monitor is unplugged, the workspace comes back to the laptop."
                        color: Qt.darker(root.bar.foreground, 1.4)
                        font.family: root.bar.fontFamily
                        font.pixelSize: Style.font.body
                    }
                    ...
                    Repeater {
                        ...
                        delegate: Item {
                            ...
                            Column {
                                ...
                                Text {
                                    width: parent.width
                                    text: root.monitorTitle(modelData)
                                    color: root.bar.foreground
                                    font.family: root.bar.fontFamily
                                    font.pixelSize: Style.font.body
                                    elide: Text.ElideRight
                                ...
                                Text {
                                    width: parent.width
                                    text: root.monitorStatus(modelData)
                            ...
                            Rectangle {
                                ...
                                anchors.verticalCenter: parent.verticalCenter
                                color: Style.hoverFillFor(root.bar.foreground, Color.accent)
                                opacity: (Number(modelData.workspace) || 0) > 0 ? 1 : 0.55
                                Text {
                                    anchors.centerIn: parent
                                    text: (Number(modelData.workspace) || 0) > 0 ? String(modelData.workspace) : "\u2014"
                                    color: root.bar.foreground
                                    font.family: root.bar.fontFamily
                                    font.pixelSize: Style.font.body
                                    font.bold: true
                                ...
                                MouseArea {
                                    ...
                                    onClicked: function(mouse) {
                                        root.cycleMonitorWorkspace(modelData, mouse.button === Qt.RightButton ? -1 : 1)
                                    }
                                }
                            }
                        }
                ...
                Column {
                    visible: !root.picking && !root.showSettings
                    width: parent.width
                    spacing: Style.space(8)
