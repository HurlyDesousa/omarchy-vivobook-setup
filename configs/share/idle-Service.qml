...
Item {
  ...
  readonly property int firstIdleTimeoutSeconds: dimTimeoutSeconds > 0 ? Math.min(dimTimeoutSeconds, screensaverTimeoutSeconds, lockTimeoutSeconds) : Math.min(screensaverTimeoutSeconds, lockTimeoutSeconds)
  ...
  function dimDisplay() {
    ...
    runProcess(dimProcess, "dim", "/home/hurly/.local/bin/omarchy-idle-dim dim")
  }
  ...
  function restoreDisplay(reason) {
    ...
    runProcess(restoreProcess, "restore-dim", "/home/hurly/.local/bin/omarchy-idle-dim restore")
  }
  ...
  function statusJson() {
    return JSON.stringify({
      ...
      dimmed: root.dimmedThisCycle,
      dimDelay: root.dimDelaySeconds,
      ...
      processes: {
        ...
        dim: dimProcess.running,
        restore: restoreProcess.running
      },
