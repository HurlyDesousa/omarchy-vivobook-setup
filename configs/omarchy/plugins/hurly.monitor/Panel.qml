...
Panel {
  ...
  // Auto-dim only. Screensaver times live in the Idle (Stay Awake) panel.
  property int idleDimSeconds: 180
  ...
  function sectionCount(section) {
    ...
    if (section === "idle") return 1        // auto-dim NumberField only
  ...
  function refresh() {
    ...
    root.loadIdleFromConfig()
  }

  function loadIdleFromConfig() {
    var idle = null
    if (root.bar && root.bar.shell && root.bar.shell.shellConfig)
      idle = root.bar.shell.shellConfig.idle
    if (!idle) return
    var dim = Number(idle.dim)
    if (isFinite(dim) && dim >= 0) root.idleDimSeconds = Math.floor(dim)
  }

  function setIdleKey(key, seconds) {
    if (key !== "dim") return
    var sec = Math.max(0, Math.floor(Number(seconds) || 0))
    root.idleDimSeconds = sec
  ...
  function commitIdleMinutes(key, text) {
    if (key !== "dim") return
    var mins = Math.max(0, Math.min(180, Math.round(Number(text) || 0)))
    root.setIdleKey("dim", mins * 60)
  }

  function adjustIdleMinutes(dx) {
    var mins = Math.round(root.idleDimSeconds / 60) + dx
    root.commitIdleMinutes("dim", mins)
  }
    ...
    PanelKeyCatcher {
      ...
      onMoveRequested: function(dx, dy) {
        ...
        if (dy !== 0) root.moveCursor(dy)
          ...
          else if (root.focusSection === "idle") root.adjustIdleMinutes(dx)
      ...
      ScrollView {
        ...
        Column {
          ...
          // ---------- Auto-dim (screensaver times are in Stay Awake / Idle) ----------
          PanelSeparator {
            foreground: root.bar.foreground
          }
          ...
          Column {
            ...
            Item {
              id: idleDimRow
              width: parent.width
              implicitHeight: Math.max(idleDimLabel.implicitHeight, idleDimField.implicitHeight, idleDimUnit.implicitHeight)
              ...
              TextField {
                ...
                anchors.right: idleDimUnit.left
                anchors.rightMargin: Style.space(6)
                anchors.verticalCenter: parent.verticalCenter
                ...
                hasCursor: root.cursorActive && root.focusSection === "idle" && root.selectedIndex === 0
                text: String(Math.round(root.idleDimSeconds / 60))
                onEditingFinished: root.commitIdleMinutes("dim", text)
                Keys.onReturnPressed: { root.commitIdleMinutes("dim", text); event.accepted = true }
                Keys.onEnterPressed: { root.commitIdleMinutes("dim", text); event.accepted = true }
                HoverHandler {
                  onHoveredChanged: if (hovered && !root.reflowingText) {
                    root.cursorActive = true
                    root.focusSection = "idle"
