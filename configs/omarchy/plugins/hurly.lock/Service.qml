...
Item {
  ...
  property bool screensaverCovering: false
  ...
  function queueSessionLock() {
    ...
    sessionLockStabilizeTimer.interval = root.screensaverCovering ? 80 : 1500
  ...
  function beginLock(withScreensaver) {
  ...
  function runWake() {
    ...
    if (lockRequested && !root.screensaverCovering) armBlankTimer()
  }
  ...
  Timer {
    ...
    onTriggered: {
      ...
      if (root.lockRequested && !root.authenticatingPassword && !root.screensaverCovering) root.runBlank()
    }
  }
  ...
  IpcHandler {
    ...
    function lock(): string {
      ...
      if (!root.beginLock(false)) return "failed"
      return "ok"
    }

    function lockIdle(): string {
      if (!root.passwordPamConfigured) return "missing-pam"
      if (root.locked || root.lockRequested) {
        if (!root.screensaverCovering) return "ok"
        return "ok"
      }
      if (!root.beginLock(true)) return "failed"
      return "ok"
    }
    ...
    function status(): string {
      return JSON.stringify({
        ...
        screensaver: root.screensaverCovering,
